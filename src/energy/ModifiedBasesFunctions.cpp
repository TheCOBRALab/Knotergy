...
int ModifiedBasesFunctions::update_energy(const LoopNode& node, const std::string& sequence,
    ...
    ModDiffs diffs = get_mod_dangle_energy_diffs(node, n5d, n3d, type, unique_mod_bases, mod_sequence, vp, mod_params, is_external);

    // Apply energy corrections based on dangle settings
    if (vp.md.dangles == 2) {
        if (n5d >=0 && n3d >=0) total_energy_diff += diffs.mismatch;
        else if (n5d >=0) total_energy_diff += diffs.n5d;
        else if (n3d >=0) total_energy_diff += diffs.n3d;
...
int ModifiedBasesFunctions::get_mod_energy(
        ...
        if (energy_lookup) {
            ...
            if (it != energy_lookup->end()) {
                std::cout << "Modified base energy found for key: " << key << " -> " << it->second 
                          << " Diff: " 
                          << static_cast<int>(it->second * 100) - unmod_energy << std::endl;
    ...
    std::cout << "No modified base energy found for key: " << key << ", using unmodified energy: " 
              << unmod_energy << " ModLookup type: " << static_cast<int>(lookup_type) << std::endl;
    return static_cast<int>(unmod_energy);
}
...
ModDiffs ModifiedBasesFunctions::get_mod_dangle_energy_diffs(
    ...
    if (n5d >=0 && n3d >=0) {
        std::string mismatch_key = join_string_views({node.begin, node.begin - 1, node.end, node.end + 1}, mod_sequence);
        int unmod_energy = is_external ? vp.p->mismatchExt[type][n5d][n3d] : vp.p->mismatchM[type][n5d][n3d];
        
        diffMM = get_mod_energy_difference(mismatch_key, unique_mod_bases, mod_params, 
                                           unmod_energy, ModLookup::Mismatch);
    }

    // Dangling 5' end only
    if (n5d >= 0) {
        std::string dangle5_key = join_string_views({node.begin, node.end, node.begin - 1}, mod_sequence);
    ...
    // Dangling 3' end only
    if (n3d >= 0) {
        std::string dangle3_key = join_string_views({node.begin, node.end, node.end + 1}, mod_sequence);
