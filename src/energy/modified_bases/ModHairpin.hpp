...
namespace knotergy {
    ...
    public:
        ...
        static int find_mod_hairpin_energy(size_t i, size_t j, const std::string& sequence,
            ...
            std::vector<std::string_view> unique_mod_bases = ModBaseUtils::unique_modified_bases_at_indices({i, j}, mod_sequence);
            ...
            return unmod_energy + ModBaseUtils::get_mod_energy(key, unique_mod_bases, mp, unmod_energy, ModLookup::TerminalAU) - vp.p->TerminalAU;
        }

    static int find_mod_hairpin_energy(const LoopNode& node, const ProcessedRNAEntry& pRNA,
                                    const std::vector<std::string_view>& mod_sequence,
                                    vrna_md_param& vp, const all_mod_params& mp, bool& is_inf) {
            return find_mod_hairpin_energy(node.begin, node.end, pRNA.get_sequence(), mod_sequence, vp, mp, is_inf);
        }
    };
}