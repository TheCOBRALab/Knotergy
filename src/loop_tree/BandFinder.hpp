...
class BandFinder {
   public:
    ...
    static std::vector<Band> find_bands(const LoopNode& node, const std::vector<size_t>& pairings,
                                        const std::vector<size_t>& cr_pairings);

   private:
    static bool extend_stem(size_t& i_prime, size_t& j_prime, const size_t& left_bound,
