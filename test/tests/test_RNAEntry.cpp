...
TEST(RNAEntryPairings, SimplePairing) {
    ...
    std::string structure = "(..)";
    compute_energy::RNAEntry rna(name, sequence, structure);
    std::vector<size_t> expected_pairings = {3, compute_energy::NULL_INDEX,
                                             compute_energy::NULL_INDEX, 0};
...
TEST(RNAEntryClosedRegions, SimpleRegion) {
    ...
    std::string structure = "(..)";
    compute_energy::RNAEntry rna(name, sequence, structure);
    std::vector<compute_energy::Region> expected_pairings = {{0, 3, false}};
...
TEST(RNAEntryClosedRegions, SimplePseudoKnot) {
    ...
    std::vector<compute_energy::Region> expected_pairings = {{0, 3, true}};
...
TEST(RNAEntryClosedRegions, PseudoknotWithNestedBP) {
    ...
    std::string structure = "[(..().])";
    compute_energy::RNAEntry rna(name, sequence, structure);
    std::vector<compute_energy::Region> expected_pairings = {{4, 5, false}, {0, 8, true}};
    EXPECT_EQ(rna.get_closed_regions(), expected_pairings);
}
...
TEST(RNAEntryClosedRegions, PseudoknotWithNestedPseudoknot) {
    ...
    std::string structure = "[(..([)].])";
    compute_energy::RNAEntry rna(name, sequence, structure);
    std::vector<compute_energy::Region> expected_pairings = {{4, 7, true}, {0, 10, true}};
    EXPECT_EQ(rna.get_closed_regions(), expected_pairings);
}
